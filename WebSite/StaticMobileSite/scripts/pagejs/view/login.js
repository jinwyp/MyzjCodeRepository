    ...
    var view = Backbone.View.extend({
        ...
        render: function () {
            ...
            Backbone.Validation.bind(this, {
                valid: function (ev, attr, selector) {
                    ev.$el.find("#" + attr + "Tip").empty().show();
                },
                invalid: function (ev, attr, error, selector) {
                    ev.$el.find("#" + attr + "Tip").text(error).show();
                }
            });
            ...
            Backbone.sync("create", this.model, {
                ...
                success: function (result) {
                    if (json.status == 1 && typeof (json.data) == "string" && json.data.length > 0) {
                        window.UpdateLoginCookie(json.info, email, json.data);
